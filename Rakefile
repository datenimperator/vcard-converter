...
desc 'Install VCardConverter.app'
task :install => [:install_app]

desc 'Install VCardConverter.app in /Applications'
task :install_app => [:uninstall_app, "xcode:install:#{DEFAULT_TARGET}:#{RELEASE_CONFIGURATION}"]

desc 'Uninstall /Applications/VCard Converter.app'
task :uninstall_app do
  ...
  if File.exists?("/Applications/VCard Converter.app")
    system("chmod", "-R", "u+w", "/Applications/VCard Converter.app")
    system("rm", "-rf", "/Applications/VCard Converter.app")
  end
end
